    ...
    itemsToShow.forEach((item) => {
        ...
        if (mode === 'smith') {
            ...
            if (g.includes('чародей') && !g.includes('ученик')) {
                ...
                sellPrice = Math.floor(sellPrice * mult); // Тут оставим как есть, так как это штучная продажа, но логика ранга тоже может быть сдвинута. Исправим для надежности.
            }
        ...
        if (g.includes('гэмблер')) sellPrice = Math.floor(sellPrice * 1.25);
            // Бонус Воришки (х1.2) только для оптовой продажи, здесь убран
            if (window.activeRiftMultiplier) {
