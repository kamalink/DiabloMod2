...
window.calculateExp = function() {
    ...
    if (g.includes('торговц')) {
        ...
        if (elitesContractCheck && elitesContractCheck.checked) {
            bonusMult = 4.0; // Итоговый множитель х5 (1 база + 4 бонус)
        }
        const bonus = eliteBossBase * bonusMult;
        runesBase += bonus; 
        paraBase += bonus;
...
window.applyExpCalculation = function() {
            ...
            if (breakChance > 0) {
                for (let i = 0; i < milestonesPassed; i++) {
                    if (Math.random() < breakChance) {
                        // Фильтруем только незаблокированные предметы
                        const breakableItems = (window.playerData.inventory || []).filter(item => !item.isLocked);
                        if (breakableItems.length > 0) {
                            const randomIndex = Math.floor(Math.random() * breakableItems.length);
                            const lostItem = breakableItems[randomIndex];
                            // Находим реальный индекс в основном инвентаре
                            const realIndex = window.playerData.inventory.indexOf(lostItem);
                            brokenItems.push(lostItem.name);
                            if (realIndex > -1) window.playerData.inventory.splice(realIndex, 1);
                        }
                    }
                }
            }
    ...
    if (!progressionTriggered) {
        window.logEvent(`Опыт: +${addRunes} Рун, +${addPara} Парагона`, 'gain');
